...
1. `GET /admin/users` → внутрішній `Path` вже без `/admin`, а `PathBase` = `/admin`.
...
4. `GET /api/ping` з `X-Api-Key: secret` → 200 — `UseWhen` повернув керування в endpoint.
5. `GET /api/whoami` з ключем → бачите `X-Api-Auth = ok`, доказ повернення в основний конвеєр.
