...
1. `GET /secret` без заголовка → `401` (коротке замикання в middleware #2, обробник не виконується).
2. `GET /secret` із `X-Api-Key: let-me-in` → проходить до кінця конвеєра.
3. `GET /nope` → `404` + рядок, дописаний middleware #3 (обробка на виході).
4. Подивіться заголовок `X-Request-Id` та лог із часом — їх додає middleware #1.
