...
має `Guid Id`, і ми порівнюємо ці Id, отримані в middleware та в дії контролера —
у межах одного запиту та між запитами.
...
з `scope.ServiceProvider` (див. дію `GET /scopes`). Так само роблять
`BackgroundService` та `IHostedService`.

### Впровадження в контролер
Дію обслуговує контролер — окремий екземпляр на кожен запит. Scoped/Singleton
беруть у **конструктор**, Transient тут узято параметрами дії з `[FromServices]`,
щоб показати два різних екземпляри в одному виклику. Явний `[FromServices]` для
`[ApiController]` не обов'язковий (джерело виводиться), але робить намір видимим.
